...
function Overview({go}:{go:(p:Page)=>void}) {
  ...
  return <div className="scroll page-pad">
    ...
    <div className="grid2">
      ...
      <section className="card panel"><div className="panel-head"><div><b>AI Agent</b><small>LangChain + LangGraph</small></div><span className="status"><span/> Agent Active</span></div><div className="agent-summary"><div className="agent-orb"><Bot size={27}/></div><div><strong>Ready to query your data</strong><p>Ask natural-language questions across SQL data and uploaded documents.</p></div></div><button className="soft" onClick={()=>go('chat')}>Open AI Chat <span>→</span></button></section>
    </div>
...
function ChatPage() {
  ...
  async function submit(e?:FormEvent) {
    ...
    try {
      let answer=''
      let meta='Demo response'
      if(api) {
        const res=await fetch(api.replace(/\/$/,'')+'/query',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({query:q})})
        if(!res.ok) throw new Error('API request failed')
        const data=await res.json()
        answer=data.response || data.answer || JSON.stringify(data,null,2)
        meta=data.queryType ? 'Query type: '+data.queryType : 'Connected backend'
      } else {
        await new Promise(r=>setTimeout(r,550))
        const lower=q.toLowerCase()
        if(lower.includes('revenue') || lower.includes('vente')) answer='Based on the selected WooCommerce source, the demo dashboard shows $84,920 revenue for the last 30 days. Connect VITE_API_URL to use the real query backend.'
        else if(lower.includes('document') || lower.includes('pdf')) answer='I can search the selected documents using the RAG workflow. In this reconstructed UI, document selection and upload are functional; connect the backend to execute real embeddings/search.'
        else answer='I understood your request: “'+q+'”. The reconstructed interface is operating in safe demo mode. Once the original compatible backend is connected, this message is sent to the /query endpoint and the live answer is displayed here.'
      }
      setMessages(v=>[...v,{id:Date.now()+1,role:'assistant',content:answer,meta}])
    } catch(err) {
      setMessages(v=>[...v,{id:Date.now()+1,role:'assistant',content:'I could not reach the configured API. The interface remains usable; verify VITE_API_URL and the backend /query endpoint.',meta:'Connection error'}])
    } finally {
      setBusy(false)
  ...
  return <div className="chat-page">
    <div className="chat-heading"><div className="agent-badge"><Zap size={14}/></div><div><div className="chat-name">AI Agent</div><div className="powered">Powered by LangChain + LangGraph <span className="status compact"><span/> Agent Active</span></div></div><div className="chat-right"><button className="debug" onClick={()=>setDebug(!debug)}>{debug?'Hide Debug':'Show Debug'}</button></div></div>
    <div className="chat-tools">
      <label><span>AI Model</span><div className="select"><Bot size={12}/><select value={model} onChange={e=>setModel(e.target.value)}><option>OpenAI GPT-4o Mini</option><option>OpenAI GPT-4o</option><option>Local Model</option></select><ChevronDown size={11}/></div></label>
    ...
    {debug&&<div className="debug-strip"><code>model={model} · database={db} · documents={doc} · backend={api?'connected':'demo'}</code></div>}
...
function ApiTest() {
  ...
  const [body,setBody]=useState('{\n  "query": "What were my top products this month?"\n}')
  const [response,setResponse]=useState('Click Send Request to preview an API response.')
  return <div className="scroll page-pad"><PageTitle eyebrow="Developer tools" title="API Test" subtitle="Validate the agent endpoint before embedding it into another application." />
    <div className="grid2"><section className="card panel"><div className="panel-head"><div><b>Request</b><small>Compatible with the original /query pattern</small></div><TestTube2 size={17}/></div><div className="form-row"><select value={method} onChange={e=>setMethod(e.target.value)}><option>POST</option><option>GET</option></select><input value={endpoint} onChange={e=>setEndpoint(e.target.value)}/></div><label className="field-label">JSON body<textarea rows={10} value={body} onChange={e=>setBody(e.target.value)}/></label><button className="primary" onClick={()=>setResponse(JSON.stringify({response:'Example AI answer from '+endpoint,timestamp:new Date().toISOString(),queryType:'demo'},null,2))}><Send size={14}/> Send Request</button></section>
    <section className="card panel"><div className="panel-head"><div><b>Response</b><small>200 OK · demo mode</small></div><span className="status"><span/> Ready</span></div><pre className="response">{response}</pre></section></div>
  </div>
}
...
function ApiKeys() {
  ...
  return <div className="scroll page-pad"><PageTitle eyebrow="Developer access" title="API Keys" subtitle="Create and revoke keys used by your widget or third-party integration." action={<button className="primary" onClick={create}><Plus size={14}/> Create key</button>} />
    <section className="card table-card"><div className="table key-head"><span>Name</span><span>Key</span><span>Created</span><span/></div>{keys.map(k=><div className="table key-row" key={k.id}><span><b>{k.name}</b></span><span className="key-value"><code>{shown===k.id?k.value:'••••••••••••••••••••••••'}</code><button onClick={()=>setShown(shown===k.id?null:k.id)}>{shown===k.id?<EyeOff size={13}/>:<Eye size={13}/>}</button></span><span>{k.created}</span><button onClick={()=>setKeys(v=>v.filter(x=>x.id!==k.id))}><Trash2 size={13}/></button></div>)}</section>
  </div>
}
...
function SettingsPage() {
  ...
  return <div className="scroll page-pad"><PageTitle eyebrow="Workspace" title="Settings" subtitle="Control agent defaults and interface preferences." action={<button className="primary" onClick={()=>{setSaved(true);setTimeout(()=>setSaved(false),1200)}}>{saved?<Check size={14}/>:<Settings size={14}/>} {saved?'Saved':'Save changes'}</button>} />
    ...
    <section className="card panel"><div className="panel-head"><div><b>Default model</b><small>Used when AI Chat opens</small></div><Sparkles size={17}/></div><div className="form-grid"><label className="field-label">Provider<select defaultValue="OpenAI"><option>OpenAI</option><option>Local / custom</option></select></label><label className="field-label">Model<select defaultValue="GPT-4o Mini"><option>GPT-4o Mini</option><option>GPT-4o</option></select></label></div></section></div>
  </div>
}
